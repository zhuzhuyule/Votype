...
| 期  | 名称                     | 状态                       | 新增 / 替换组件                                                      |
| --- | ------------------------ | -------------------------- | -------------------------------------------------------------------- |
| A   | 插入可靠性地基           | ✅ A1 · ⛔ A2 待开工       | 新增 insert_guard（已交付）；替换 clipboard 光标探测占位实现（待做） |
...
| A2 AX 可编辑文本快照 | Vokie `native/editable-text-snapshot`（fullText/selectedRange/elementId，失败返回 `no_focused_element`） | 替换 `clipboard.rs::get_selected_text` / `get_cursor_context` 现有占位/弱实现：插入前快照焦点可编辑区；失败给**结构化原因**而非空串。衔接：上游总纲 Phase 3 paste_tx 的 receipt 校验以 A2 快照为"插入前状态"，两案在 paste_tx 落地时合并 |
