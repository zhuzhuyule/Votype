...
import { ExternalSkillsList } from "./prompts/components/ExternalSkillsList";
...
const PromptsConfiguration: React.FC = () => {
  ...
  const [skillsTab, setSkillsTab] = useState<"quick" | "advanced">("quick");

  // External skills from ~/.votype/skills/
  const {
    externalSkills,
      ...
      <Flex direction="column" gap="5">
        ...
        {enabled && (
          <Card className="p-0! overflow-hidden border-gray-100 dark:border-gray-800 shadow-sm rounded-xl">
            <Tabs.Root
              value={skillsTab}
              onValueChange={(v) => setSkillsTab(v as "quick" | "advanced")}
            >
              <Tabs.List className="border-b border-gray-100 dark:border-gray-800">
                <Tabs.Trigger value="quick" className="cursor-pointer">
                  {t("settings.postProcessing.skills.quickCommands")}
                </Tabs.Trigger>
                <Tabs.Trigger value="advanced" className="cursor-pointer">
                  {t("settings.postProcessing.skills.advancedSkills")}
                </Tabs.Trigger>
              </Tabs.List>

              {/* Tab 1: Quick Commands (existing prompts UI) */}
                        ...
                        <SidebarItem
                          option={{
                            ...
                            label:
                              draftName ||
                              t(
                                "settings.postProcessing.prompts.newPromptName",
                              ),
                          }}
                          isActive={false}
                          isSelected={true}
                          ...
                          <Flex gap="2" align="center">
                            ...
                            {!isCreating && currentTab !== activePromptId && (
                              ...
                              >
                                <IconStar size={14} />
                                {t(
                                  "settings.postProcessing.prompts.setAsActive",
                                )}
                              </Button>
                            )}
              ...
              {/* Tab 2: Advanced Skills (external skills from folder) */}
              <Tabs.Content value="advanced">
                <ExternalSkillsList
                  skills={externalSkills}
                  isLoading={isLoadingExternal}
                  onRefresh={refreshExternalSkills}
                  onOpenFolder={openSkillsFolder}
                />
              </Tabs.Content>
            </Tabs.Root>
          </Card>
