...
struct PendingRelease {
    ...
    /// Hold (push-to-talk) passes zero so every release stops.
...
fn classify_busy_input(
    ...
    match (mode, is_pressed, remembered) {
        ...
        (Hold | HoldOrToggle, true, None) => BusyAction::Remember,
        (Hold | HoldOrToggle, true, Some(Remembered::Locked)) => BusyAction::Forget,
        (Hold | HoldOrToggle, true, Some(Remembered::Held)) => BusyAction::Ignore,
        ...
        (Hold | HoldOrToggle, false, _) => BusyAction::Ignore,
    }
}
...
impl InputEvent {
    ...
    fn effective_hold_threshold(&self) -> Duration {
        match self.mode {
            ActivationMode::HoldOrToggle => self.hold_threshold,
            // Every release stops; toggle never defers releases at all.
            ActivationMode::Hold | ActivationMode::Toggle => Duration::ZERO,
        }
    }
}
...
///
/// * hold (push-to-talk) — every release stops (hold threshold of zero)
...
impl CoordinatorState {
    ...
    fn on_input(&mut self, input: InputEvent, now: Instant) -> Option<Effect> {
        ...
        let hold_to_talk = input.mode != ActivationMode::Toggle && !self.is_locked();
            ...
            match classify_busy_input(input.is_pressed, input.mode, remembered) {
                BusyAction::Remember => {
                    ...
                    self.pending_press = Some(PendingPress {
                        ...
                        locked: input.mode == ActivationMode::Toggle,
                        binding_id: input.binding_id,
                        hotkey_string: input.hotkey_string,
                        pressed_at: now,
                    });
        ...
        if input.is_pressed {
            match &self.stage {
                Stage::Idle => {
                    // Toggle never ends on a release: locked from the start.
                    let locked = input.mode == ActivationMode::Toggle;
                ...
                Stage::Recording(id) if id == &input.binding_id => {
                    ...
                    if self.is_locked() || input.mode == ActivationMode::Toggle {
...
impl TranscriptionCoordinator {
    ...
    /// only matters for [`ActivationMode::HoldOrToggle`].
    pub fn send_input(
        &self,
        binding_id: &str,
        hotkey_string: &str,
...
mod tests {
    ...
    fn toggle_press_during_processing_remembers_start() {
        assert_eq!(
            classify_busy_input(true, ActivationMode::Toggle, None),
            BusyAction::Remember
        );
    }
    ...
    fn second_toggle_press_during_processing_forgets_press() {
        assert_eq!(
            classify_busy_input(true, ActivationMode::Toggle, Some(Remembered::Locked)),
            BusyAction::Forget
        );
    }
    ...
    fn toggle_release_during_processing_is_ignored() {
        assert_eq!(
            classify_busy_input(false, ActivationMode::Toggle, None),
            BusyAction::Ignore
        );
        assert_eq!(
            classify_busy_input(false, ActivationMode::Toggle, Some(Remembered::Locked)),
            BusyAction::Ignore
        );
    }
    ...
    fn hold_modes_classify_busy_inputs_by_pending_state() {
        ...
        for mode in [ActivationMode::Hold, ActivationMode::HoldOrToggle] {
    ...
    fn toggle_presses_alternate_remember_and_forget_while_busy() {
        ...
        for expected in [
            ...
            let action = classify_busy_input(true, ActivationMode::Toggle, remembered);
    ...
    /// Hold press while the pipeline is busy is remembered and starts recording
    ...
    const OTHER_BINDING: &str = "invoke_skill";
    ...
    fn different_binding_does_not_replace_pending_press() {
        ...
        assert!(
            effect.is_none(),
            "two transcribe toggles net to no-op; the ignored skill toggle must not start"
        );
        assert_eq!(state.stage, Stage::Idle);
    }
    ...
    fn input(mode: ActivationMode, is_pressed: bool) -> InputEvent {
        InputEvent {
            binding_id: BINDING.to_string(),
            hotkey_string: BINDING.to_string(),
            is_pressed,
    ...
    fn hold_or_toggle_long_hold_stops_on_release() {
        let mode = ActivationMode::HoldOrToggle;
        let mut state = CoordinatorState::new();
        let t0 = Instant::now();
    ...
    fn toggle_press_stops_recording_started_as_hold() {
        ...
        state.on_input(input(ActivationMode::HoldOrToggle, true), t0);
        assert!(!state.is_locked());
        assert!(matches!(
            state.on_input(input(ActivationMode::Toggle, true), t0 + ms(2000)),
            Some(Effect::Stop { .. })
        ));
    }
