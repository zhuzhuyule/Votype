...
mod sherpa_engine {
    ...
    impl SherpaOnnxOnlineRecognizer {
        ...
        pub(super) fn decode(&self, sample_rate: i32, samples: &[f32]) -> Result<String> {
            ...
            while offset < samples.len() {
                ...
                unsafe {
                    ...
                    while sherpa_safe::SafeSherpaOnnxIsOnlineStreamReady(self.recognizer, stream)
                        == 1
                    {
                        if sherpa_safe::SafeSherpaOnnxDecodeOnlineStream(self.recognizer, stream)
                            != 1
                        {
            ...
            let result_ptr = unsafe {
                sherpa_safe::SafeSherpaOnnxGetOnlineStreamResult(self.recognizer, stream)
            };
        ...
        pub(super) fn is_endpoint(
            &self,
            stream: *const sherpa_rs_sys::SherpaOnnxOnlineStream,
        ) -> bool {
            unsafe {
                sherpa_safe::SafeSherpaOnnxOnlineStreamIsEndpoint(self.recognizer, stream) == 1
            }
        }

        pub(super) fn reset_stream(&self, stream: *const sherpa_rs_sys::SherpaOnnxOnlineStream) {
            let ok =
                unsafe { sherpa_safe::SafeSherpaOnnxOnlineStreamReset(self.recognizer, stream) };
            if ok != 1 {
    ...
    impl SherpaOnnxOfflineRecognizer {
        ...
        pub(super) fn decode(&self, sample_rate: i32, samples: &[f32]) -> Result<String> {
            ...
            if unsafe { sherpa_safe::SafeSherpaOnnxDecodeOfflineStream(self.recognizer, stream) }
                != 1
            {
                unsafe { sherpa_safe::SafeSherpaOnnxDestroyOfflineStream(stream) };
