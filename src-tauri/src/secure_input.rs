...
mod imp {
    ...
    mod tests {
        ...
        fn binding(id: &str, current: &str) -> ShortcutBinding {
            ShortcutBinding {
                id: id.to_string(),
                name: id.to_string(),
        ...
        #[test]
        fn modifier_only_bindings_are_immune() {
            let b = binding("transcribe", "command_left");
            assert!(matches!(
                plan_fallback_binding("transcribe", &b),
                ShadowPlan::Immune
            ));
        }

        #[test]
        fn fn_key_bindings_cannot_be_covered() {
            let b = binding("transcribe", "fn+f13");
            assert!(matches!(
                plan_fallback_binding("transcribe", &b),
        ...
        #[test]
        fn full_modifier_shadow_keeps_semantics() {
            let b = binding("transcribe", "ctrl+alt+shift+command+space");
            match plan_fallback_binding("transcribe", &b) {
                ShadowPlan::Shadow { shadow, degraded } => {
                    assert!(!degraded, "both sides specified — nothing widened");
                    let original: Hotkey = b.current_binding.parse().unwrap();
                    let parsed: Hotkey = shadow.current_binding.parse().unwrap();
                    assert_eq!(original, parsed);
                }
                other => panic!("expected shadow plan, got {other:?}"),
            }
        }

        #[test]
        fn side_specific_modifiers_widen_to_degraded() {
            let b = binding("transcribe", "option_left+space");
            match plan_fallback_binding("transcribe", &b) {
                ShadowPlan::Shadow { shadow, degraded } => {
                    assert!(degraded, "left-only option widens to either side");
                    let parsed: Hotkey = shadow.current_binding.parse().unwrap();
                    assert!(parsed.modifiers.contains(Modifiers::OPT));
        ...
        #[test]
        fn mouse_bindings_are_immune() {
            let b = binding("transcribe", "ctrl+mouseleft");
            assert!(matches!(
                plan_fallback_binding("transcribe", &b),
                ShadowPlan::Immune
            ));
        ...
        #[test]
        fn unparseable_bindings_report_uncovered() {
            let b = binding("transcribe", "definitely not a shortcut");
            assert!(matches!(
                plan_fallback_binding("transcribe", &b),
                ShadowPlan::Uncovered
            ));
