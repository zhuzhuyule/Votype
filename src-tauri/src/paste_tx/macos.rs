...
define_class!(
    ...
    #[name = "VotypePasteProvider"]
    #[ivars = ProviderIvars]
    pub struct VotypePasteProvider;

    impl VotypePasteProvider {
...
struct MacPending {
    ...
    provider: Option<Retained<VotypePasteProvider>>,
    auto_submit: bool,
        ...
        match enigo {
            ...
            None => {
                if let Some(enigo_state) = app_handle.try_state::<EnigoState>() {
                    if let Ok(mut guard) = enigo_state.0.lock() {
...
pub(super) fn run(
    ...
    let provider = VotypePasteProvider::new(state.clone(), text.to_string());
